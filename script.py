...
def summarize_document(text, max_retries=3):
    """
    Summarize the given document using GPT with retry mechanism.
...
def generate_question_answer_pairs(text, num_pairs=5, max_retries=3):
    """
    Generate question-answer pairs from the document with retry mechanism.
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(f"Q&A generation attempt {attempt + 1} failed: {str(e)}")
            if attempt == max_retries - 1:
                logger.error(f"Failed to generate Q&A pairs after {max_retries} attempts")
                return []
...
def classify_document(text):
    """
    Classify document into categories based on content.
    ...
    categories = {
        'technical': ['algorithm', 'code', 'software', 'programming', 'api', 'function', 'database'],
        'business': ['revenue', 'profit', 'market', 'customer', 'sales', 'strategy', 'growth'],
        'academic': ['research', 'study', 'hypothesis', 'methodology', 'analysis', 'conclusion'],
        'news': ['reported', 'announced', 'yesterday', 'today', 'breaking', 'sources'],
        'creative': ['story', 'character', 'plot', 'narrative', 'imagine', 'dream']
    }
...
def display_enhanced_analysis(topics, sentiment, entities, readability, classification=None):
    """Display the results of enhanced analysis."""
    print("\n📊 Enhanced Analysis Results:\n" + "-"*60)
    
    if classification:
        print("\n📁 Document Classification:")
        print(f"- Primary Category: {classification['primary_category'].capitalize()}")
        print(f"- Confidence: {classification['confidence']:.2%}")
        print("- Category Scores:")
        for cat, score in classification['category_scores'].items():
            print(f"  • {cat}: {score}")
...
class DocumentProcessor:
    ...
    def chunk_document(self, text):
        ...
        BUG: The chunk size comparison uses >= instead of >, which may cause
        chunks to be smaller than necessary when exactly at the limit.
        """
        sentences = sent_tokenize(text)
        chunks = []
        current_chunk = []
    ...
    def process_chunk(self, chunk_text):
        ...
        return {
            ...
            'classification': classify_document(chunk_text)
        }
    ...
    def _combine_chunk_results(self, chunk_results):
        ...
        for result in chunk_results:
            if 'classification' in result:
                for cat, score in result['classification']['category_scores'].items():
                    category_totals[cat] += score
        
        if category_totals:
            combined['classification']['category_scores'] = dict(category_totals)
            combined['classification']['primary_category'] = max(category_totals, key=category_totals.get)
            total_score = sum(category_totals.values())
            combined['classification']['confidence'] = max(category_totals.values()) / total_score if total_score > 0 else 0
